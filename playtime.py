...
class Playtime:
    ...
    def identify_directories(
        ...
        for relpath in directories:
            ...
            # get/update extra metadata?
            cgng = self.cgng_online_imdb_lookup(title=title)
            if cgng is None:
                continue
    ...
    def cgng_online_imdb_lookup(
        ...
        # get new data
        try:
            data = imdb.get_title(title.title_id, accept_language="en")
        except Exception:
        ...
        cgng, created = CGNGData.objects.update_or_create(title_id=title.title_id, defaults={"data": modeldata})
        if created:
            logger.debug(f"Downloaded extra metadata for title {title}")
        else:
            logger.debug(f"Updated extra metadata for title {title}")
        return cgng
    ...
    def get_title_categories(self, title: Title, category: str) -> list[str]:
        ...
        elif category == "top_ranking":
            things = [title.cgngdata.data["top_ranking"]] if hasattr(title, "cgngdata") else []
        elif category == "bottom_ranking":
            things = [title.cgngdata.data["bottom_ranking"]] if hasattr(title, "cgngdata") else []
        else:
            things = []
        return things
    ...
    def create_symlink_dirs(self, *, symlink_dir: Path, categories: list[str], relative: bool = True) -> None:
        ...
        for category in categories:
            ...
            logger.info(f"Creating symlinks by {category} in {categorydir}...")
    ...
    def get_category_subdir(self, categorydir: Path, category: str, thing: str, title: Title) -> Path:
        """Return the subdir to use for this category."""
        subdir: Path
        if category in ["actors", "directors", "producers", "composers", "writers"]:
            # make a level with the first letter
            subdir = categorydir / str(thing)[0] / str(thing) / title.dirname
        elif category == "runtime":
            step = 30
            runtime = int(thing) * step
            subdir = categorydir / f"{runtime}-{runtime + step}" / title.dirname
        elif category == "rating":
...
def get_parser() -> argparse.ArgumentParser:
    ...
    ###########################################
    # playtime extrameta
    subparsers.add_parser("extrameta", help="Get/update extra metadata (cover+ranking) from IMDB.")
